(function () {
	...
	function init () {
		...
		fbStudents.child("001").on("value", function(snapshot){
			...
			// update the viewModelMap with the new data
			ko.mapping.fromJS(data, viewModelMap);
			console.log	(viewModelMap);
			// console.log(viewModelMap.name() + " test name 2");
		...
		fbMe.on("value", function(snapshot){
			...
			// console.log(viewModelMap.me.info());

			// update the viewModelMap with the new data
			ko.mapping.fromJS(data, viewModelMap);
			// console.log(data);
			console.log(viewModelMap);
			console.log(viewModelMap.info());
		});

		// create the viewModelMap from our initial data
		viewModelMap = ko.mapping.fromJS({
			...
			setPage: function (clickedPage) {
				viewModelMap.currentPage(clickedPage);
				console.log("clicked a page, now on " + viewModelMap.currentPage());
			},
